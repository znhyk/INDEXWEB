function make_raw_data_array(raw_data) {
    return raw_data.replace(/\s/g, '').replace(/\[/g, '').replace(/\]/g, '').replace(/\,/g, '').replace(/\'\'/g, ',').replace(/\'/g, '').split('\,');
}
...
function make_card(data_array, year, month, day) {
    ...
    for (var i = 0; i < data_array.length; i++) {
        var temp = "";
        temp = "<a href=\"/gpu_index_500/specific/" + data_array[i][0] + "/" + year + "/" + month + "\"><div class=\"col-xl-10 col-md-10 mb-4\"><div class=\"card border-left-primary shadow h-100 py-2\"><div class=\"card-body\"><div class=\"row no-gutters align-items-center\"><div class=\"col mr-2\"><div class=\"text-xs font-weight-bold text-primary text-uppercase mb-1\">";
        temp += data_array[i][1];
